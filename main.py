...
try:
    ...
    if args.read_json:
        node_list = item.List.from_json(nodes_file, tree_file, args.config_file)
        ...
        for element in elements:
            item = item.List.make_node(element, tree, config)
    ...
    while current_node:
        ...
        if isinstance(current_node, item.SetPointNode):
            #print(f"append {next_node.name()} to {current_node.name()}")
            current_node.links.append(next_node)
        if isinstance(next_node, item.SetPointNode):
    ...
    for item in node_list:
        if (isinstance(item, item.SetPointNode)):
    ...
    if args.save_json:
        f = open(nodes_file, "w")
        json.dump(node_list, f, cls=item.ListEncoder, indent=4)
        f.close()
